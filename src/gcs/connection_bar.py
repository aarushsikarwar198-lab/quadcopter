...
class ConnectionBar(QWidget):
    ...
    def set_link_state(self, state: str) -> None:
        if state not in (OFFLINE, CONNECTING, LIVE):
            # was an `assert` — asserts are silently stripped when Python
            # runs with `-O`, which would let an invalid state through
            # uncaught instead of failing loudly right here.
            raise ValueError(f"invalid link state: {state!r}")
    ...
    def _refresh(self) -> None:
        ...
        if connecting:
            ...
            self.connect_btn.setToolTip("Cancel this connection attempt.")
        elif self._link_state == LIVE:
            self.connect_btn.setText("Disconnect")
            self.connect_btn.setToolTip("Disconnect from the current bridge link.")
        else:
