...
def grid_label(cx: int, cy: int) -> str:
    ...
    Coerces to int up front: JSON doesn't distinguish 3 from 3.0, so a
    bridge is free to send cell coordinates as floats. Without this,
    `n % 26` and `n // 26` on a float eventually produce a float, and
    `chr()` raises TypeError on that — which used to be able to break
    map rendering for every cell, every frame, until a non-float
    coordinate came in (map_canvas.paintEvent now also catches stray
    exceptions like this, but fixing it here means coordinate tags
    keep working instead of silently disappearing)."""
    cx = int(cx)
    cy = int(cy)
    neg = cx < 0
    n = abs(cx)
...
class MapCanvas(QWidget):
    ...
    def paintEvent(self, event):
        ...
        try:
            ...
            # Message handling (handle_message in main_window.py) is
            # wrapped in try/except, but paintEvent runs later, on Qt's
            # own redraw schedule, entirely outside that protection. A
            # single mistyped field slipping through from any bridge —
            # e.g. a non-numeric x/y — would otherwise raise here and
            # could take the whole window down mid-render. Skipping just
            # this one frame and logging is far safer than crashing.
            print(f"[map_canvas] paint error (frame skipped): {exc!r}")
        finally:
            p.end()
    ...
    def _paint(self, p: QPainter):
        ...
        for room in self.state.rooms.values():
            sx, sy = self.world_to_screen(room.get("x", 0), room.get("y", 0))
        ...
        for obj in self.state.objects.values():
            sx, sy = self.world_to_screen(obj.get("x", 0), obj.get("y", 0))
        ...
        for role, wp in self.state.waypoints.items():
            sx, sy = self.world_to_screen(wp.get("x", 0), wp.get("y", 0))
            cxp, cyp = sx + s / 2, sy + s / 2
            r = max(7, s * 0.3)
        ...
        for sv in self.state.survivors.values():
            sx, sy = self.world_to_screen(sv.get("x", 0), sv.get("y", 0))
            cxp, cyp = sx + s / 2, sy + s / 2
