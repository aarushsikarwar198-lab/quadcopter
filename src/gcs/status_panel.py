...
class StatusPanel(QFrame):
    def __init__(self, parent=None):
        ...
        def add_stat(row, col, key, attr_name):
            ...
            # QLabel defaults to auto-detecting rich text: if a value we
            # set here ever looks like it contains HTML (e.g. "phase"
            # arriving from a bridge as something containing a "<"),
            # it would get interpreted as markup instead of shown
            # literally. These always hold plain telemetry values, so
            # pin the format explicitly rather than relying on Qt's guess.
